...
*/
contract AutopayV2 is UsingFetchUpgradeReady, Initializable, OwnableUpgradeable, UUPSUpgradeable {
    // Storage
    ...
    uint256 public newUpgradeUint;
    bytes32 public newUpgradeBytes;
    ...
    function testNewUpgradeFunction() external view returns (uint256) {
        return block.timestamp + newUpgradeUint;
    }

    function testChangeBytes(bytes32 _newValue) external {
        newUpgradeBytes = _newValue;
    }

    function testChangeUint(uint256 _newValue) external {
        newUpgradeUint = _newValue;
    }
