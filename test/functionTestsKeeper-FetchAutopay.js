...
describe("KEEPER - function tests", () => {
    let fetch,autopay,accounts,blocky,jobParamsList,jobId,queryIdParams,queryData,queryId,
    ...
    it("claimJobTips", async () => {
        ...
        let keeperBalB4 = await fetch.balanceOf(accounts[1].address);
        let ownerBal = await fetch.balanceOf(autopay.owner())
        await fetch.connect(accounts[1]).submitValue(queryId,valSub,0,queryData);
        ...
        expect(await fetch.balanceOf(accounts[1].address)).to.equal(keeperBalB4.add(maxGasCover.add(tip).sub((maxGasCover.add(tip).mul(10)).div(1000))));// keeper gets gas + tip minus 1 percent fee
        expect(await fetch.balanceOf(autopay.owner())).to.equal(ownerBal.add(fee));// owner collects 1 percent fee
    ...
    it("fundJob", async () => {
        await autopay.initKeeperJob(functionSig,h.zeroAddress,80001,blocky.timestamp,maxGasCover,400,600,tip);
        let autopayBal = await fetch.balanceOf(autopay.address);
        let BAL = await fetch.balanceOf(accounts[0].address);
        await expect(autopay.fundJob(jobId,tip)).to.emit(autopay, "KeeperJobFunded").withArgs(accounts[0].address,tip,jobId);
        expect(await fetch.balanceOf(accounts[0].address)).to.equal(BAL.sub(maxGasCover.add(tip)));
        expect(await fetch.balanceOf(autopay.address)).to.equal(autopayBal.add(maxGasCover.add(tip)));
    });
    ...
    it("increaseMaxGasForExistingJob", async () => {
        ...
        queryParams = abiCoder.encode(queryIdParamTypes,queryIdParams);
        queryData = abiCoder.encode(["string","bytes"],[TYPE,queryParams]);
        let autopayBefore = await fetch.balanceOf(autopay.address);
        await autopay.increaseMaxGasForExistingJob(keccak256(queryData), maxGasCover);
        expect(await fetch.balanceOf(autopay.address)).to.equal(autopayBefore.add(maxGasCover));
    ...
    it("keeperClaimTip", async () => {
        ...
        queryParams = abiCoder.encode(queryIdParamTypes,queryIdParams);
        queryData = abiCoder.encode(["string","bytes"],[TYPE,queryParams]);
        let fetchBalB4 = await fetch.balanceOf(fetch.address); // oracle
        let acct1Bal = await fetch.balanceOf(accounts[1].address); // keeper
        let VAL = abiCoder.encode(["bytes32","address","uint256","uint256"],[keccak256("0x"),accounts[1].address,callTime.timestamp,maxGasCover]);
        await fetch.connect(accounts[1]).submitValue(keccak256(queryData),VAL,0,queryData);
        ...
        assert(BigInt(await fetch.balanceOf(fetch.address)) > fetchBalB4);// oracle increased balance cause of fee
        assert(BigInt(await fetch.balanceOf(accounts[1].address)) > acct1Bal);
    });
    ...
    it("tipKeeperJob", async () => {
        let autopayBalance = await fetch.balanceOf(autopay.address);
        let acctBeforeBalance = await fetch.balanceOf(accounts[0].address);
        ...
        expect(await fetch.balanceOf(accounts[0].address)).to.equal(acctBeforeBalance.sub(acctBalDiff));
        expect(await fetch.balanceOf(autopay.address)).to.equal(autopayBalance.add(acctBalDiff));
    });
...
//     //     expect(await fetch.balanceOf(autopay.address)).to.equal(autopayBefore.sub(MAXGASCOVER.add(web3.utils.toWei("1"))));
//     // });
});